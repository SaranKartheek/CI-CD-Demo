pipeline{
            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'nexus_password', variable: 'nexus_password')]) {
                             sh '''
                                docker build -t 18.222.188.214:8083/springapp:${DOCKER_TAG} .
                                docker login -u admin -p $nexus_password 18.222.188.214:8083
                                docker push 18.222.188.214:8083/springapp:${DOCKER_TAG}
				docker logout
			    '''
		    }
		}
	...
	stage("docker image push to dockerhub and remove the images locally"){
            steps{
				...
				docker tag 18.222.188.214:8083/springapp:${DOCKER_TAG} saranvemireddy/springapp:${DOCKER_TAG}
                                docker push saranvemireddy/springapp:${DOCKER_TAG}
				docker rmi 18.222.188.214:8083/springapp:${DOCKER_TAG}
                    ...
                    withCredentials([string(credentialsId: 'nexus_password', variable: 'nexus_password')]) {
                          dir('kubernetes/') {
                             sh '''
                                  ...
                                  curl -u admin:$nexus_password http://18.222.188.214:8081/repository/helm-repo/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
			    ...
			    script{
					...
					echo "${DOCKER_TAG}"
					        sh '''
								 ...
								 sudo scp -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa ${WORKSPACE}/kubernetes/manifests-yamls/*.yaml root@172.31.3.82:/etc/ansible/kubernetes/
							'''     
				}
            }
        }			
        ...
        stage('Deploying application on k8s cluster from ansible-server using playbook') {
            steps {
               script{
			        sh 'ssh -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa root@172.31.3.82 "kubectl config get-contexts; kubectl config use-context kubernetes-admin@kubernetes; whoami && hostname; ansible-playbook  /etc/ansible/hosts /etc/ansible/kubernetes/playbook-deployment-service.yaml; sudo rm -rf /etc/ansible/kubernetes/*.yaml;"'   
               }
            }
        }
